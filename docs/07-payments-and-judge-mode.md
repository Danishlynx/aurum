...
- JUDGE_PER_SESSION_CAPS decides whether the four per session caps below refuse anything. It is on for exactly "true" and off for every other value, unset included, and unset is the default. Off is a decision. Every per session cap is per owner, and an owner costs nothing to mint: the code is published, each submission of it makes a session with a fresh set of caps, so they never bounded the account. What they bounded was the one person who cannot mint a way around them, the founder, who re entered the code 44 times in one afternoon. With the switch off the counters still move, the ledger still records every unit, and /api/judge/stats and the cap log lines still read true numbers; only the refusals are gone. The judge banner goes with them: it is a live count and a promise about what happens when that count runs out, and with the switch off nothing happens when it runs out, so the banner is hidden rather than rewritten. The brakes that remain are GLOBAL_CAP_PERFECTCORP_UNITS_PER_DAY across the whole deployment and DAILY_CAP_PERFECTCORP_UNITS per owner per UTC day, and neither reads this switch. Set it to "true" to restore the judging behaviour the per session bullets below describe, exactly as it shipped.
- Each capture that reaches the analyze step decrements analyses_used. With the switch on, and when analyses_used equals analyses_allowed, the capture screen is disabled with the flow doc copy and reads serve the demo profile. With it off, analyses_used goes on counting past analyses_allowed, the capture screen stays live, and reads stay on the session's own rows, because a session that may take a photo must see its own reading rather than the demo profile's face.
- Every Perfect Corp call during a judge session reserves units against credits_cap. With the switch on, exceeding the cap returns 429 and the app falls back to cache or demo, so a judge is never stranded on a broken screen. With it off, credits_used still moves past credits_cap and nothing is refused for it.
- SerpApi searches have their own per session allowance, JUDGE_SERPAPI_SEARCHES (default 40), counted from the session's own ledger rows rather than from credits_cap. It refuses only while the switch is on. A unit and a search are different currencies bought from different companies, and credits_cap is sized in units: while they shared one counter, a session that had finished its analyses could not buy a single search, and every routine step on the report read "No listing found near you yet". Claude calls are recorded and not capped, as they always were on the daily cap.
- Above every cap above sits one that is not per owner: GLOBAL_CAP_PERFECTCORP_UNITS_PER_DAY, the Perfect Corp units this whole deployment may spend in a UTC day, counted across every owner's ledger rows at once and checked before any per owner cap. It exists because an owner is free. The access code is published, each submission of it mints a session with a fresh credits_cap and a fresh daily cap, and on 2026-09-12 that made 44 sessions in one afternoon and took the account from 502 units to 100 with every per owner cap respected. The default is 560, ten full capture sets of 56 units, which is more than a demo day needs and cheap enough that a runaway costs at most one day of it. Set the variable from the balance the account actually holds; the env value wins. Refunds are negative ledger rows, so a task the engine failed gives its units back to the ceiling. Only Perfect Corp is bounded this way: SerpApi has its own per owner daily cap and its own plan quota, and Claude is recorded rather than capped. A reservation past the ceiling returns the same 429 and the same sentence as the per owner day, logged as global_credits so the two can be told apart, and GET /api/health reports perfectcorpGlobalRemainingToday so the number can be watched during judging.
- Renders are limited to 12 per judge session while JUDGE_PER_SESSION_CAPS is on, and to nothing per session while it is off. Raised from 6 on 2026-09-03: one demo session spends a render on each of the four makeup rows, a hairstyle, a hair color, and a garment, and a judge who hits the cap mid screen sees the same empty hero as a judge with no photo. Every render still reserves its units, so the daily cap and the deployment wide ceiling still price it either way.
