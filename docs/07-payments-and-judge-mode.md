...
- Above every cap above sits one that is not per owner: GLOBAL_CAP_PERFECTCORP_UNITS_PER_DAY, the Perfect Corp units this whole deployment may spend in a UTC day, counted across every owner's ledger rows at once and checked before any per owner cap. It exists because an owner is free. The access code is published, each submission of it mints a session with a fresh credits_cap and a fresh daily cap, and on 2026-09-12 that made 44 sessions in one afternoon and took the account from 502 units to 100 with every per owner cap respected. The default is 460, ten full capture sets, which is more than a demo day needs and cheap enough that a runaway costs at most one day of it. Set the variable from the balance the account actually holds; the env value wins. Refunds are negative ledger rows, so a task the engine failed gives its units back to the ceiling. Only Perfect Corp is bounded this way: SerpApi has its own per owner daily cap and its own plan quota, and Claude is recorded rather than capped. A reservation past the ceiling returns the same 429 and the same sentence as the per owner day, logged as global_credits so the two can be told apart, and GET /api/health reports perfectcorpGlobalRemainingToday so the number can be watched during judging.
