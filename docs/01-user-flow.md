...
Layout: a 3:4 camera stage, the column's full width (on a window too short to hold it under the header and above the controls, the stage shrinks its width, centred, rather than cropping the frame). The stage shows exactly the master frame below, mirrored the way a person expects of a camera pointed at them. A soft oval frame in antique gold hairline marks where the face should be, drawn from the frame's own geometry. Below the stage, one line of live guidance. A single shutter control, disabled until the camera has delivered a frame the geometry could be read from. A small "Upload instead" text link for people without a working camera. The video, the still that freezes at the shutter and the oval share one mirrored wrapper, so the still is the mirror image the person framed and does not flip at the tap; the frame that is uploaded is drawn from the un mirrored video, so the picture the analysis reads keeps a mole on its own cheek.

The master frame contract, decided 2026-09-23 and in force since the master frame landed. There is one portrait 3:4 frame with fixed geometry, identical on every device: it is what the person sees on the stage, it is what the live line and the gate measure, and it is what is uploaded. The oval is the target face inside that frame, 0.70 of the frame's width edge to edge at the cheekbones (the visible face, which is what the engine's rule measures), 1.35 times as tall as it is wide, centred at 0.50 of the width and 0.47 of the height, and it is defined in the frame's own coordinates rather than drawn over whatever the camera granted. The landmarker's own cheek to cheek span sits about a fifth inside that visible edge (0.55 of the frame when the face fills the oval, measured on the founder's phone on 2026-09-23), so every width the gate reads is the span divided by that share (MESH_FACE_WIDTH_SHARE, 0.79) and a person who fills the oval reads 0.70; the raw span is stored beside it for the calibration report. A phone track keeps its full width as a centred 3:4 crop (1080 by 1920 gives 1080 by 1440); a laptop track keeps its full height (1920 by 1080 gives 810 by 1080). The geometry is read from a delivered frame, never from a timer, because iOS reports the landscape sensor size for the first few hundred milliseconds of a track and then the portrait size; it is read again whenever the track changes size. The upload path composes a gallery photo into the same geometry around the face it finds, and the one reframe retry is a concentric crop of that frame that cannot cut inside the oval. Every number lives in src/lib/shared/frame-geometry.ts and is derived from the engine's four gate families and the Camera Kit presets in docs/04-integrations.md, so a face that fills the oval is the same face, at the same share of the picture, on an iPhone, an Android and a webcam alike.
...
- "Hold still."
- "Good. Tap to capture."
- "Good. Hold still." (frame turns solid gold; the photo is about to take itself)
...
The photo takes itself, with the tap as the fallback (decided with the founder 2026-09-23). After 800 ms of "Good. Tap to capture." without a break, the oval turns solid gold and the line becomes "Good. Hold still."; 700 ms later the shutter fires on its own. The countdown cancels the moment the line leaves "ready", and the oval goes back to the hairline. The shutter works at any time, before, during and after the hold, so a person who would rather tap still can, and a screen reader hears the change through the line, which is live.
...
When the face model has not loaded, the line says so instead of guessing: the two light lines over the whole frame ("Face the light" and "Too bright"), then "Hold still.", then "The face check did not load. You can still take the photo." The tap still works; the frame is offered unmeasured (below).
...
- A face is measured by the face model (MediaPipe's FaceLandmarker: 478 landmarks, a solved head pose, the eye blink blendshapes), or the frame is unmeasured because the model did not load. There is nothing in between: no colour rule, no browser detector. An unmeasured frame is never refused on the face, because nothing looked at one; it is offered with "Use it anyway" and the reason "unmeasured", and the engine's own input gate, which is free, decides. The one thing that still refuses an unmeasured frame is light over the whole frame at the extremes: a frame black or white over its whole area is refused as too dark or over exposed, measured or not, because nothing can be read off it by anyone.
- On a measured frame, exactly one face. No face, or more than one, is a refusal.
- The face at least 60 percent of the frame width, the visible face edge to edge, which is the rule the engine itself applies; it is read off the model's cheek to cheek span divided by the measured share of the visible face that span covers (0.79, the master frame contract above). Under the Camera Kit RELAXED floor of 0.55 the frame is refused as too far; between 0.55 and 0.60 it is offered; above 0.86 it is offered as too close. The frame judged is the master frame that is sent: the camera's own crop, or, for an uploaded photo, the same geometry composed around the face the model found. The live line is measured on a sample of that same frame and holds a little inside the gate: "Move closer" under 0.64 and "Move back" above 0.85, the oval's own band, so that "Good" is said for a face that fills the oval and not for one the gate would merely let through.
...
The framing rule was face height against frame height. The engine's rule is face width against the frame's short axis, and a face sitting exactly on our height rule in a 3 by 4 frame lands at about 0.56 of the width where the engine wants more than 0.60. Our gate passed at precisely the value the engine refuses at. Both rules were then checked, and a frame that failed the width one was recomposed around the face rather than refused, the camera path doing what the upload path had done since 2026-09-02. Since the master frame the camera path sends the frame as shown and only the upload path composes (above).
...
- "The face check did not load on this device, so the photo was not checked." (offered, never refused on the face; only the whole frame light extremes above still refuse it)
...
Behavior: the shutter, tapped or fired by the hold, takes a short burst of 3 master crops about 90ms apart, drawn straight from the video with the master rect as the source rect, not the single frame at the instant of the tap, because the instant of the tap is the instant the finger pressing the glass moves the phone and this product gets one attempt at a reading. Each frame is measured once, on a 512px copy (the face model, the light over the oval, the evenness between the eyes, the blink and the sharpness all from that copy; nothing reads pixels at master size), and the best scoring one is sent (frameScore in src/lib/shared/quality.ts, which ranks on pose first, then framing, then the eyes, then light, then sharpness). When every frame of the burst has both eyes shut, up to three more frames are taken before choosing. When the camera track changes size inside the burst, or the face's width moves by more than 15 percent between frames, the burst is discarded and the shutter re armed with "Hold the phone upright". The frame frozen on screen becomes the winner, and the winner's reading is the reading its stored row carries: the losers travel with it as numbers only. On accept, the winner is encoded at the master frame's native size (3:4, long edge capped at 1440, short edge at least 480; no downscale to 1024), which strips EXIF, then hashed, uploaded to the private captures bucket, and the analysis jobs start. Route to /analyzing.

An uploaded photo goes the same way with one step in front: the model reads a 1024px copy of it, and the photo is composed into the master geometry around the face it found (masterCropFor in src/lib/shared/frame-geometry.ts: the face at the oval's width and centre, never narrower than the face, slid inside the picture rather than shrunk), cut from the file at full resolution and drawn with the same cap and floor. A file the browser cannot decode whose header says HEIC or HEIF is answered with "This photo format cannot be read here. Pick a JPEG or take a new one." rather than an upload failure, since nothing was uploaded; the file input still accepts image/* so an iPhone hands over a JPEG on its own.

## E. Analyzing (/analyzing)

Purpose: the reveal. This is the signature moment of the product and of the demo video.

Layout: the person's captured selfie fills the screen, mirrored the way they framed it on the capture stage, slightly darkened at the edges by a single radial vignette centred on the oval (0.50 of the width, 0.47 of the height of the frame). As each analysis returns, its masks bloom over the face in translucent antique gold, then settle; the still and the masks are mirrored together, so a mask aligned to the uploaded frame stays on the pixels it was measured on. Below, one line of status.
