...
**C2. Add the eval faces.** `evals/fixtures/faces/f01-window.jpg` through `f12-indoor.jpg`, two captures of each person on the same day, one in good window light and one in warm indoor light, as 3:4 master frames with the long edge at most 1440 and EXIF already stripped. Add `evals/fixtures/faces/labels.json` with the human labelled Fitzpatrick type, undertone, and hair type per face. The full rule, including which sources of faces are allowed, is `evals/fixtures/README.md`.
...
The face luma bands and the exposure fractions in `src/lib/shared/quality.ts`, and the width bands and the pose window in `src/lib/shared/frame-geometry.ts` and `quality.ts`, are numbers about real photographs. Set them from the calibration report of C3 against the bar in `docs/05-evals.md`: a constant moves only when a row of that report says the engine refuses on the other side of it, and precision of accept against the engine stays at or above 0.97. Sharpness has no threshold at all: it is read at `SHARPNESS_MEASURE_LONG_EDGE`, recorded with the capture and used to rank the frames of a burst, and it neither refuses nor flags a frame, because the engine publishes no blur code and its own input gate reads a soft frame for free.
