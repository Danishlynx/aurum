...
The bad frame categories (dark, over exposed, off center, partial face, no face, turned away, eyes closed, two faces, unmeasured) have no photo folder. They are covered by the synthetic frames eval:capture builds itself, on synthetic landmarker readings (evals/support/synthetic-face.ts), and by the exported outcome rows under evals/fixtures/private (below), where the engine's own refusals name what the gate missed. Blurry is not a category: softness is recorded in captures.quality and ranks the frames of a burst, and is never refused or flagged, because the engine publishes no blur code. A photo of a printed photo is not a category the gate claims to catch.
...
- Runs the client quality gate logic (extracted as a pure function over image tensors) against synthetic frames, one per failure category, which proves the decision logic, the reason precedence, and the accept and borderline boundaries.
- Runs the server side byte validation (evals/capture/validate.test.ts over src/lib/server/capture/validate.ts and the pure JPEG header reader in src/lib/shared/jpeg-header.ts): a PNG, a wrong size, a short side under 480, a file over 10 MB and a digest that does not match are each refused before any reservation, and a matching 1080 by 1440 JPEG passes.
- The fixture half runs on exported outcome rows rather than on photographs. npm run calibration:export (service role key, never in CI) joins captures.quality to what the engine did with each first camera capture (attempt 1, path camera; rows touched by a provider failure are left out) and writes evals/fixtures/private/capture-outcomes.json, which is gitignored and holds numbers only: the gate's measurements, the client verdict, each reading's status, refusal code and elapsed time, and the engine's own face_quality words. The suite parses that file against a schema and skips with a printed reason when it is absent.
- Metric: precision and recall of "accept" against "the engine accepted every runnable kind" (skin, Fitzpatrick, attributes, face shape). Precision is P(engine accepted every kind | stored verdict accept); recall is P(stored verdict accept | engine accepted every kind). Both are reported and written to evals/results/capture-<git sha>.json. Precision is asserted at or above 0.97 only once at least 100 decided rows exist; below that it is reported, because a rate over fewer rows would not move a threshold honestly.
- Thresholds move only from these rows. npm run calibration:report prints, for each measured number (absolute yaw, signed pitch, absolute roll, face width ratio, face luma, blink), the engine's acceptance rate by bucket, then every refusal code with our numbers beside it, and writes evals/results/capture-calibration-<git sha>.json. A constant in src/lib/shared/quality.ts changes only when a row of that report says the engine refuses on the other side of it, and the PR attaches the report.
- The one go reading rate is the number the whole pipeline is judged on: over first camera captures the client accepted and a face model measured, with no provider failure, the share where all four runnable readings succeeded, the last of them within 120 seconds of the capture, and the profile points at the capture. A capture with no readings at all (its analyze never ran) counts as a miss. It is computed by the capture_outcomes view (migration 0015), which carries the platform word, and reported by /api/judge/stats over the last seven days, leaving out captures younger than 120 seconds. The calibration report prints the engine's acceptance of our accepted frames per platform (ios, android, desktop) so a threshold that fails on one kind of camera is not hidden by the others. Zero failures in 100 says at least 96 percent; 99 percent needs about 300 to 380 clean first captures.
- The founder's private selfies under evals/fixtures/private/faces (gitignored, consented, listed in the plan) are for a Playwright gate eval that runs the real gate over each through "Upload instead" and records our numbers. That eval is not built yet.
