...
- The one go reading rate is the number the whole pipeline is judged on: over first camera captures the client accepted and a face model measured, with no provider failure, the share where all four runnable readings succeeded and the profile pointed at the capture within 120 seconds. It is computed by the capture_outcomes view (migration 0015), reported by /api/judge/stats over the last seven days, and stratified by platform in the calibration report. Zero failures in 100 says at least 96 percent; 99 percent needs about 300 to 380 clean first captures.
